const presence = new Presence({
  clientId: '1337020534516486166',
})
const userInfo = JSON.parse(JSON.parse(localStorage['persist:nt']).user)
...
presence.on('UpdateData', async () => {
  if (!pageVariables) {
    pageVariables = await presence.getPageVariable<PageVarsType>(
  ...
  switch (pathArr[1]) {
    ...
    case 'leagues': {
      ...
      const leaguePlacement = getNumberWithOrdinal(
        Number.parseInt(
    ...
    case 'play': {
      presenceData.details = `Playing problem type:`
      presenceData.state = pageVariables['NTGLOBALS.LESSON_DESCRIPTIONS'].find(lesson => lesson.lessonID === userInfo.lessonID)?.shortDescription
